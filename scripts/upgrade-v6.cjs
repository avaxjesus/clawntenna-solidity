...
async function main() {
  ...
  console.log("🔄 Upgrading AntennaRegistry to V6");
  ...
  const registryV5 = await ethers.getContractAt("AntennaRegistryV5", REGISTRY_PROXY);
  const currentVersion = await registryV5.getVersion();
  ...
  // Upgrade to V6
  // ==========================================
  console.log("\n⬆️ Upgrading to AntennaRegistryV6...");

  const AntennaRegistryV6 = await ethers.getContractFactory("AntennaRegistryV6");

  // Force import if not registered (for previously deployed proxies)
  console.log("   Checking proxy registration...");
  try {
    await upgrades.validateUpgrade(REGISTRY_PROXY, AntennaRegistryV6);
    console.log("   ✅ Already registered");
  } catch (e) {
    if (e.message.includes("not registered")) {
      console.log("   Importing existing proxy...");
      const CurrentFactory = await ethers.getContractFactory("AntennaRegistryV5");
  ...
  const registryV6 = await upgrades.upgradeProxy(REGISTRY_PROXY, AntennaRegistryV6, {
    kind: 'uups',
    unsafeSkipStorageCheck: false
  });
  await registryV6.waitForDeployment();
  console.log("   ✅ Upgrade transaction complete");

  const newImpl = await upgrades.erc1967.getImplementationAddress(REGISTRY_PROXY);
  const newVersion = await registryV6.getVersion();
  ...
  // Verify version
  if (newVersion !== "6.0.0") {
    throw new Error(`Version mismatch! Expected 6.0.0, got ${newVersion}`);
  }
  ...
  // ==========================================
  console.log("\n" + "═".repeat(50));
  console.log("📶 ANTENNA V6 UPGRADE COMPLETE");
  console.log("═".repeat(50));
  console.log("\nChanges in V6:");
  console.log("  ✅ getAgentTokenId validates live ownership via ownerOf");
  console.log("  ✅ hasAgentIdentity delegates to validated getAgentTokenId");
  console.log("  ✅ Stale registrations (transferred tokens) return 0");
  console.log("\nAddresses:");
  console.log("  Proxy (unchanged):", REGISTRY_PROXY);
  ...
  deployment.v6Features = {
    validatedAgentIdentityLookups: true
  };
  ...
  console.log(`   npx hardhat run scripts/upgrade-v6.cjs --network ${otherNetwork}`);
}
